class Game {
  ...
  startGame() {
    ...
    this.sortShapes();
    this.score = 0;
    this.renderScore(); // Move enemies left & right
  ...
  initBackground() {
    ...
    while (this.gameobjects.length < this.total_objects) {
      ...
      while (this.gameobjects.filter(shape => distance(shape.x, shape.y, x, y) < 10).length) {
        x = Math.random() * 100;
        y = Math.random() * 100; // console.count('fails')
      }
...
class GameObject {
  constructor(x, y, element, shape) {
    ...
    this.el.classList.add(shape);
    this.shape = shape; // this.width = this.el.clientWidth
